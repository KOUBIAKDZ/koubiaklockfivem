                    ...
                    if(string.lower(plate) == localVehPlate)then
                        ...
                        if(vehicle ~= "locked")then
                            ...
                            if(time > timer)then
                                ...
                                TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Yavaş ol dostum, kilidi kıracaksın' } )
                            end
                        else
                            TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Anahtarlar içeride değil' } )
                        end
                    end
                end
...
AddEventHandler("ls:getHasOwner", function(hasOwner, localVehId, localVehPlate, localVehLockStatus)
    if(not hasOwner)then
        ...
        TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Anahtarları torpidodan aldın' } )
    else
        TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Bu araç sizin değil' } )
    end
end)
...
function Notify(text, duration)
	if(globalConf['CLIENT'].notification)then
		if(globalConf['CLIENT'].notification == 1)then
			...
			Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_LIFEINVADER", "CHAR_LIFEINVADER", true, 1, "Centralisation V" .. _VERSION, "by koubiak", duration)
			DrawNotification_4(false, true)
		elseif(globalConf['CLIENT'].notification == 2)then
			TriggerEvent('chatMessage', '^Centralisation V' .. _VERSION, {255, 255, 255}, text)
		else
			return
		end
	else
